...
export function PlaylistCard({ 
  ...
  return (
    <div 
      className={`bg-spotify-lightgray rounded-lg p-4 hover:bg-gray-700 transition-colors group relative ${
        bulkSelectMode ? 'cursor-pointer' : ''
      } ${isSelected ? 'ring-2 ring-spotify-green' : ''}`}
      onClick={() => bulkSelectMode && onToggleSelect?.(playlist.id)}
    >
      {}
      {bulkSelectMode && (
      ...
      <div className="aspect-square mb-4 rounded-md overflow-hidden bg-gray-800">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={playlist.name}
